...
#ifndef theta_sketch_dup_DUP_IMPL_HPP_
#define theta_sketch_dup_DUP_IMPL_HPP_
...
template<typename A>
typename theta_sketch_dup_alloc<A>::unique_ptr theta_sketch_dup_alloc<A>::deserialize(std::istream& is, uint64_t seed) {
  ...
  if (type == update_theta_sketch_dup_alloc<A>::SKETCH_TYPE) {
    typename update_theta_sketch_dup_alloc<A>::resize_factor rf = static_cast<typename update_theta_sketch_dup_alloc<A>::resize_factor>(preamble_longs >> 6);
    typedef typename std::allocator_traits<A>::template rebind_alloc<update_theta_sketch_dup_alloc<A>> AU;
    return unique_ptr(
      static_cast<theta_sketch_dup_alloc<A>*>(new (AU().allocate(1)) update_theta_sketch_dup_alloc<A>(update_theta_sketch_dup_alloc<A>::internal_deserialize(is, rf, lg_cur_size, lg_nom_size, flags_byte, seed))),
      [](theta_sketch_dup_alloc<A>* ptr) {
        ptr->~theta_sketch_dup_alloc();
        AU().deallocate(static_cast<update_theta_sketch_dup_alloc<A>*>(ptr), 1);
      }
    );
  } else if (type == compact_theta_sketch_dup_alloc<A>::SKETCH_TYPE) {
    typedef typename std::allocator_traits<A>::template rebind_alloc<compact_theta_sketch_dup_alloc<A>> AC;
    return unique_ptr(
      static_cast<theta_sketch_dup_alloc<A>*>(new (AC().allocate(1)) compact_theta_sketch_dup_alloc<A>(compact_theta_sketch_dup_alloc<A>::internal_deserialize(is, preamble_longs, flags_byte, seed_hash))),
      [](theta_sketch_dup_alloc<A>* ptr) {
        ptr->~theta_sketch_dup_alloc();
        AC().deallocate(static_cast<compact_theta_sketch_dup_alloc<A>*>(ptr), 1);
      }
    );
  }
...
template<typename A>
typename theta_sketch_dup_alloc<A>::unique_ptr theta_sketch_dup_alloc<A>::deserialize(const void* bytes, size_t size, uint64_t seed) {
  ...
  if (type == update_theta_sketch_dup_alloc<A>::SKETCH_TYPE) {
    ...
    typedef typename std::allocator_traits<A>::template rebind_alloc<update_theta_sketch_dup_alloc<A>> AU;
    return unique_ptr(
      static_cast<theta_sketch_dup_alloc<A>*>(new (AU().allocate(1)) update_theta_sketch_dup_alloc<A>(
        update_theta_sketch_dup_alloc<A>::internal_deserialize(ptr, size - (ptr - static_cast<const char*>(bytes)), rf, lg_cur_size, lg_nom_size, flags_byte, seed))
      ),
      [](theta_sketch_dup_alloc<A>* ptr) {
        ptr->~theta_sketch_dup_alloc();
        AU().deallocate(static_cast<update_theta_sketch_dup_alloc<A>*>(ptr), 1);
    ...
    typedef typename std::allocator_traits<A>::template rebind_alloc<compact_theta_sketch_dup_alloc<A>> AC;
    return unique_ptr(
      static_cast<theta_sketch_dup_alloc<A>*>(new (AC().allocate(1)) compact_theta_sketch_dup_alloc<A>(
        compact_theta_sketch_dup_alloc<A>::internal_deserialize(ptr, size - (ptr - static_cast<const char*>(bytes)), preamble_longs, flags_byte, seed_hash))
      ),
      [](theta_sketch_dup_alloc<A>* ptr) {
        ptr->~theta_sketch_dup_alloc();
        AC().deallocate(static_cast<compact_theta_sketch_dup_alloc<A>*>(ptr), 1);
...
template<typename A>
void theta_sketch_dup_alloc<A>::check_sketch_type(uint8_t actual, uint8_t expected) {
  if (actual != expected) {
...
template<typename A>
void theta_sketch_dup_alloc<A>::check_serial_version(uint8_t actual, uint8_t expected) {
  if (actual != expected) {
...
template<typename A>
void theta_sketch_dup_alloc<A>::check_seed_hash(uint16_t actual, uint16_t expected) {
  if (actual != expected) {
...
template<typename A>
update_theta_sketch_dup_alloc<A>::update_theta_sketch_dup_alloc(uint8_t lg_cur_size, uint8_t lg_nom_size, resize_factor rf, float p, uint64_t seed):
...
template<typename A>
update_theta_sketch_dup_alloc<A>::update_theta_sketch_dup_alloc(bool is_empty, uint64_t theta, uint8_t lg_cur_size, uint8_t lg_nom_size, vector_u64<A>&& keys, uint32_t num_keys, resize_factor rf, float p, uint64_t seed):
...
void update_theta_sketch_dup_alloc<A>::serialize(std::ostream& os) const {
  ...
  os.write((char*)keys_.data(), sizeof(std::pair<uint64_t,uint64_t>) * keys_.size());
}
...
vector_u8<A> update_theta_sketch_dup_alloc<A>::serialize(unsigned header_size_bytes) const {
  ...
  ptr += copy_to_mem(keys_.data(), ptr, sizeof(std::pair<uint64_t, uint64_t>) * keys_.size());

  return bytes;
}

template<typename A>
update_theta_sketch_dup_alloc<A> update_theta_sketch_dup_alloc<A>::deserialize(std::istream& is, uint64_t seed) {
  uint8_t preamble_longs;
  is.read((char*)&preamble_longs, sizeof(preamble_longs));
  ...
  theta_sketch_dup_alloc<A>::check_serial_version(serial_version, theta_sketch_dup_alloc<A>::SERIAL_VERSION);
  theta_sketch_dup_alloc<A>::check_seed_hash(seed_hash, theta_sketch_dup_alloc<A>::get_seed_hash(seed));
...
template<typename A>
update_theta_sketch_dup_alloc<A> update_theta_sketch_dup_alloc<A>::internal_deserialize(const void* bytes, size_t size, resize_factor rf, uint8_t lg_cur_size, uint8_t lg_nom_size, uint8_t flags_byte, uint64_t seed) {
...
template<typename A>
void update_theta_sketch_dup_alloc<A>::update(const void* data, unsigned length) {
  HashState hashes;
...
template<typename A>
compact_theta_sketch_dup_alloc<A> update_theta_sketch_dup_alloc<A>::compact(bool ordered) const {
...
void update_theta_sketch_dup_alloc<A>::resize() {
  ...
  for (uint32_t i = 0; i < keys_.size(); i++) {
    if (keys_[i].first != 0) {
      hash_search_or_insert(keys_[i].first, new_keys.data(), lg_new_size); // TODO hash_insert
    }
  }
...
void update_theta_sketch_dup_alloc<A>::rebuild() {
  ...
  for (uint32_t i = 0; i < keys_.size(); i++) {
    if (keys_[i].first != 0 && keys_[i].first < this->theta_) {
      hash_search_or_insert(keys_[i].first, new_keys.data(), lg_cur_size_); // TODO hash_insert
      num_keys_++;
    }
  }
  keys_ = std::move(new_keys);
...
template<typename A>
uint32_t update_theta_sketch_dup_alloc<A>::get_capacity(uint8_t lg_cur_size, uint8_t lg_nom_size) {
  const double fraction = (lg_cur_size <= lg_nom_size) ? RESIZE_THRESHOLD : REBUILD_THRESHOLD;
  return std::floor(fraction * (1 << lg_cur_size));
}

template<typename A>
uint32_t update_theta_sketch_dup_alloc<A>::get_stride(uint64_t hash, uint8_t lg_size) {
...
template<typename A>
bool update_theta_sketch_dup_alloc<A>::hash_search_or_insert(uint64_t hash, std::pair<uint64_t, uint64_t>* table, uint8_t lg_size) {
...
template<typename A>
bool update_theta_sketch_dup_alloc<A>::hash_search(uint64_t hash, const std::pair<uint64_t,uint64_t>* table, uint8_t lg_size) {
  const uint32_t mask = (1 << lg_size) - 1;
  const uint32_t stride = update_theta_sketch_dup_alloc<A>::get_stride(hash, lg_size);
  uint32_t cur_probe = static_cast<uint32_t>(hash) & mask;
  const uint32_t loop_index = cur_probe;
  do {
    const uint64_t value = table[cur_probe].first;
...
template<typename A>
typename theta_sketch_dup_alloc<A>::const_iterator update_theta_sketch_dup_alloc<A>::begin() const {
  return typename theta_sketch_dup_alloc<A>::const_iterator(keys_.data(), keys_.size(), 0);
}

template<typename A>
typename theta_sketch_dup_alloc<A>::const_iterator update_theta_sketch_dup_alloc<A>::end() const {
  return typename theta_sketch_dup_alloc<A>::const_iterator(keys_.data(), keys_.size(), keys_.size());
}

// compact sketch

template<typename A>
compact_theta_sketch_dup_alloc<A>::compact_theta_sketch_dup_alloc(bool is_empty, uint64_t theta, vector_u64<A>&& keys, uint16_t seed_hash, bool is_ordered):
theta_sketch_dup_alloc<A>(is_empty, theta),
keys_(std::move(keys)),
...
template<typename A>
compact_theta_sketch_dup_alloc<A>::compact_theta_sketch_dup_alloc(const theta_sketch_dup_alloc<A>& other, bool ordered):
...
void compact_theta_sketch_dup_alloc<A>::serialize(std::ostream& os) const {
  ...
  const uint8_t flags_byte(
    ...
    (this->is_empty() ? 1 << theta_sketch_dup_alloc<A>::flags::IS_EMPTY : 0) |
    (this->is_ordered() ? 1 << theta_sketch_dup_alloc<A>::flags::IS_ORDERED : 0)
  );
  os.write((char*)&flags_byte, sizeof(flags_byte));
  const uint16_t seed_hash = get_seed_hash();
  os.write((char*)&seed_hash, sizeof(seed_hash));
...
template<typename A>
vector_u8<A> compact_theta_sketch_dup_alloc<A>::serialize(unsigned header_size_bytes) const {
  const bool is_single_item = keys_.size() == 1 && !this->is_estimation_mode();
  const uint8_t preamble_longs = this->is_empty() || is_single_item ? 1 : this->is_estimation_mode() ? 3 : 2;
  const size_t size = header_size_bytes + sizeof(uint64_t) * preamble_longs + sizeof(std::pair<uint64_t, uint64_t>) * keys_.size();
  vector_u8<A> bytes(size);
...
template<typename A>
compact_theta_sketch_dup_alloc<A> compact_theta_sketch_dup_alloc<A>::deserialize(std::istream& is, uint64_t seed) {
  uint8_t preamble_longs;
  is.read((char*)&preamble_longs, sizeof(preamble_longs));
  uint8_t serial_version;
  is.read((char*)&serial_version, sizeof(serial_version));
...
template<typename A>
compact_theta_sketch_dup_alloc<A> compact_theta_sketch_dup_alloc<A>::internal_deserialize(std::istream& is, uint8_t preamble_longs, uint8_t flags_byte, uint16_t seed_hash) {
  ...
  if (!is_empty) is.read((char*)keys.data(), sizeof(std::pair<uint64_t, uint64_t>) * keys.size());

  const bool is_ordered = flags_byte & (1 << theta_sketch_dup_alloc<A>::flags::IS_ORDERED);
  if (!is.good()) throw std::runtime_error("error reading from std::istream"); 
  return compact_theta_sketch_dup_alloc<A>(is_empty, theta, std::move(keys), seed_hash, is_ordered);
}

template<typename A>
compact_theta_sketch_dup_alloc<A> compact_theta_sketch_dup_alloc<A>::deserialize(const void* bytes, size_t size, uint64_t seed) {
  ensure_minimum_memory(size, 8);
  const char* ptr = static_cast<const char*>(bytes);
  uint8_t preamble_longs;
  ptr += copy_from_mem(ptr, &preamble_longs, sizeof(preamble_longs));
...
template<typename A>
compact_theta_sketch_dup_alloc<A> compact_theta_sketch_dup_alloc<A>::internal_deserialize(const void* bytes, size_t size, uint8_t preamble_longs, uint8_t flags_byte, uint16_t seed_hash) {
  const char* ptr = static_cast<const char*>(bytes);
  const char* base = ptr;
...
template<typename A>
typename theta_sketch_dup_alloc<A>::const_iterator compact_theta_sketch_dup_alloc<A>::begin() const {
  return typename theta_sketch_dup_alloc<A>::const_iterator(keys_.data(), keys_.size(), 0);
}

template<typename A>
typename theta_sketch_dup_alloc<A>::const_iterator compact_theta_sketch_dup_alloc<A>::end() const {
  return typename theta_sketch_dup_alloc<A>::const_iterator(keys_.data(), keys_.size(), keys_.size());
}
...
template<typename A>
typename update_theta_sketch_dup_alloc<A>::builder& update_theta_sketch_dup_alloc<A>::builder::set_lg_k(uint8_t lg_k) {
  if (lg_k < MIN_LG_K) {
...
template<typename A>
typename update_theta_sketch_dup_alloc<A>::builder& update_theta_sketch_dup_alloc<A>::builder::set_resize_factor(resize_factor rf) {
  rf_ = rf;
  return *this;
}

template<typename A>
typename update_theta_sketch_dup_alloc<A>::builder& update_theta_sketch_dup_alloc<A>::builder::set_p(float p) {
  p_ = p;
  return *this;
}

template<typename A>
typename update_theta_sketch_dup_alloc<A>::builder& update_theta_sketch_dup_alloc<A>::builder::set_seed(uint64_t seed) {
  seed_ = seed;
  return *this;
}

template<typename A>
uint8_t update_theta_sketch_dup_alloc<A>::builder::starting_sub_multiple(uint8_t lg_tgt, uint8_t lg_min, uint8_t lg_rf) {
  return (lg_tgt <= lg_min) ? lg_min : (lg_rf == 0) ? lg_tgt : ((lg_tgt - lg_min) % lg_rf) + lg_min;
}

template<typename A>
update_theta_sketch_dup_alloc<A> update_theta_sketch_dup_alloc<A>::builder::build() const {
  return update_theta_sketch_dup_alloc<A>(starting_sub_multiple(lg_k_ + 1, MIN_LG_K, static_cast<uint8_t>(rf_)), lg_k_, rf_, p_, seed_);
}

// iterator

template<typename A>
theta_sketch_dup_alloc<A>::const_iterator::const_iterator(const std::pair<uint64_t, uint64_t>* keys, uint32_t size, uint32_t index):
keys_(keys), size_(size), index_(index) {
  while (index_ < size_ && keys_[index_].first == 0) ++index_;
}

template<typename A>
typename theta_sketch_dup_alloc<A>::const_iterator& theta_sketch_dup_alloc<A>::const_iterator::operator++() {
  do {
    ++index_;
...
template<typename A>
typename theta_sketch_dup_alloc<A>::const_iterator theta_sketch_dup_alloc<A>::const_iterator::operator++(int) {
...
template<typename A>
bool theta_sketch_dup_alloc<A>::const_iterator::operator==(const const_iterator& other) const {
...
template<typename A>
std::pair<uint64_t, uint64_t> theta_sketch_dup_alloc<A>::const_iterator::operator*() const {
  return keys_[index_];
}
...
template<typename A>
void update_theta_sketch_dup_alloc<A>::remove(const void* data, unsigned length) {
  HashState hashes;
  MurmurHash3_x64_128(data, length, seed_, hashes);
  const uint64_t hash = hashes.h1 >> 1; // Java implementation does logical shift >>> to make values positive
...
template<typename A>
bool update_theta_sketch_dup_alloc<A>::hash_search_or_remove(uint64_t hash, std::pair<uint64_t, uint64_t>* table, uint8_t lg_size) {
  const uint32_t mask = (1 << lg_size) - 1;
  const uint32_t stride = update_theta_sketch_dup_alloc<A>::get_stride(hash, lg_size);
  uint32_t cur_probe = static_cast<uint32_t>(hash) & mask;
  const uint32_t loop_index = cur_probe;
