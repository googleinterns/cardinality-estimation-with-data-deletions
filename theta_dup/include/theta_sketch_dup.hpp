...
#ifndef THETA_SKETCH_DUP_HPP_
#define THETA_SKETCH_DUP_HPP_
...
template<typename A> class theta_sketch_dup_alloc;
template<typename A> class update_theta_sketch_dup_alloc;
template<typename A> class compact_theta_sketch_dup_alloc;
...
public:
  ...
  typedef std::unique_ptr<theta_sketch_dup_alloc<A>, std::function<void(theta_sketch_dup_alloc<A>*)>> unique_ptr;
...
template<typename A> using AllocU64 = typename std::allocator_traits<A>::template rebind_alloc<std::pair<uint64_t,uint64_t>>;
template<typename A> using vector_u64 = std::vector<std::pair<uint64_t, uint64_t>, AllocU64<A>>;
...
public:
   ...
   */
  static update_theta_sketch_dup_alloc<A> deserialize(std::istream& is, uint64_t seed = DEFAULT_SEED);
...
private:
  ...
  /**
   * keys_ stores a vector of pairs: the first element in pairs represent the hash value,
   * the second element in pairs represent the number of times this element shows up in the stream
   */
  vector_u64<A> keys_;
  uint32_t num_keys_;
  resize_factor rf_;
  ...
  // for builder
  update_theta_sketch_dup_alloc(uint8_t lg_cur_size, uint8_t lg_nom_size, resize_factor rf, float p, uint64_t seed);

  // for deserialize
  update_theta_sketch_dup_alloc(bool is_empty, uint64_t theta, uint8_t lg_cur_size, uint8_t lg_nom_size, vector_u64<A>&& keys, uint32_t num_keys, resize_factor rf, float p, uint64_t seed);
  ...
  /**
   * search hash values, if exists increase the count and return true, otherwise insert the value and return false
   * @param hash: the hash value
   * @param table: the pointer to the hash table
   * @param table: lg_size of the current hash table
   */
  static bool hash_search_or_insert(uint64_t hash, std::pair<uint64_t, uint64_t>* table, uint8_t lg_size);
  /**
   * search hash values, if exists decrease the count
   * if the count==0, remove the element from hash table and return true else return false
   * @param hash: the hash value
   * @param table: the pointer to the hash table
   * @param table: lg_size of the current hash table
   */
  static bool hash_search_or_remove(uint64_t hash, std::pair<uint64_t, uint64_t>* table, uint8_t lg_size);
  static bool hash_search(uint64_t hash, const std::pair<uint64_t, uint64_t>* table, uint8_t lg_size);

  friend theta_sketch_dup_alloc<A>;
  static update_theta_sketch_dup_alloc<A> internal_deserialize(std::istream& is, resize_factor rf, uint8_t lg_cur_size, uint8_t lg_nom_size, uint8_t flags_byte, uint64_t seed);
  static update_theta_sketch_dup_alloc<A> internal_deserialize(const void* bytes, size_t size, resize_factor rf, uint8_t lg_cur_size, uint8_t lg_nom_size, uint8_t flags_byte, uint64_t seed);
};
...
public:
  ...
  compact_theta_sketch_dup_alloc(const theta_sketch_dup_alloc<A>& other, bool ordered);
   ...
   * @return an instance of a sketch
   */
  static compact_theta_sketch_dup_alloc<A> deserialize(std::istream& is, uint64_t seed = DEFAULT_SEED);
   ...
   * @return an instance of the sketch
   */
  static compact_theta_sketch_dup_alloc<A> deserialize(const void* bytes, size_t size, uint64_t seed = DEFAULT_SEED);
...
private:
  ...
  // friend theta_intersection_alloc<A>;
  // friend theta_a_not_b_alloc<A>;
  compact_theta_sketch_dup_alloc(bool is_empty, uint64_t theta, vector_u64<A>&& keys, uint16_t seed_hash, bool is_ordered);
  static compact_theta_sketch_dup_alloc<A> internal_deserialize(std::istream& is, uint8_t preamble_longs, uint8_t flags_byte, uint16_t seed_hash);
  static compact_theta_sketch_dup_alloc<A> internal_deserialize(const void* bytes, size_t size, uint8_t preamble_longs, uint8_t flags_byte, uint16_t seed_hash);
};
...
// iterator
template<typename A>
class theta_sketch_dup_alloc<A>::const_iterator: public std::iterator<std::input_iterator_tag, std::pair<uint64_t, uint64_t>> {
public:
...
private:
  ...
  uint32_t size_;
  uint32_t index_;
  const_iterator(const std::pair<uint64_t, uint64_t>* keys, uint32_t size, uint32_t index);
  friend class update_theta_sketch_dup_alloc<A>;
  friend class compact_theta_sketch_dup_alloc<A>;
};


// aliases with default allocator for convenience (std::allocator<void> is no longer supported in c++20)
typedef theta_sketch_dup_alloc<std::allocator<void>> theta_sketch_dup;
typedef update_theta_sketch_dup_alloc<std::allocator<void>> update_theta_sketch_dup;
typedef compact_theta_sketch_dup_alloc<std::allocator<void>> compact_theta_sketch_dup;
...
#include "theta_sketch_dup_impl.hpp"
