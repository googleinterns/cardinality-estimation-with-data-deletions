...
class gen_string {
  ...
  public:
    ...
    gen_string(int& seed0, int& min_len0, int& max_len0): seed(seed0), min_len(min_len0), max_len(max_len0){
      gen.seed(seed);
      generate_pool();
      dist_code=std::uniform_int_distribution<int>(0,pool.size()-1);
      dist_len=std::uniform_int_distribution<int>(min_len,max_len);
    ...
    // @max_len=max_len0
    // @pool=pool0
    gen_string(int& seed0, int& min_len0, int& max_len0, std::vector<char>& pool0): seed(seed0), min_len(min_len0), max_len(max_len0), pool(pool0){
      gen.seed(seed);
      dist_code=std::uniform_int_distribution<int>(0,pool.size()-1);
      dist_len=std::uniform_int_distribution<int>(min_len,max_len);
    }
